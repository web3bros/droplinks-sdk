import {DropLink, DropLinkClaim, DropLinksSearchOptions, NewDropLinkOptions} from "./types";
...
export class DropLinks {
  ...
  private static async request(method: string, url: string, data: object | undefined = undefined, params: object | undefined = undefined): Promise<any> {
    return axios({
      method,
      url: `${this.apiUrl}${url}`,
      data,
      ...
      error => {
        console.error(`droplinks.io returned ${error.response.status}: ${error.response?.data?.error || error.response.statusText}`)
        throw error
      }
    )
  }
  ...
  public static async getDropLinks(options: DropLinksSearchOptions = {}): Promise<DropLink[]> {
    return this.request(
      'GET',
      '/drop-links',
      {},
...
export {
  DropLink, DropLinkClaim, DropLinksSearchOptions, NewDropLinkOptions
}